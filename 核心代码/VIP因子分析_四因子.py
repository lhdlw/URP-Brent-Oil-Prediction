...
class RobustPLS:
    ...
    def _calc_metrics(self, X, Y):
        ...
        # 计算VIP - 每个变量有4个特征
        self.vip_scores = np.zeros(n_features // 4)
        total_exp_Y = sum([v['Y'] for v in self.explained_variance.values()])

        if total_exp_Y > 0:
            for var_idx in range(n_features // 4):
                vip_sum = 0
                ...
                self.vip_scores[var_idx] = np.sqrt((n_features // 4) * vip_sum / total_exp_Y)


def analyze(opec_file, sp500_file, dollar_file, vix_file, brent_file):
    """
    主分析函数：4个影响因子 → 伦敦布伦特原油
    """

    print("=" * 60)
    print("伦敦布伦特原油期货价格影响因素分析（4因子）")
    print("=" * 60)
    ...
    print("\n步骤1: 加载数据")
    print("-" * 60)
    opec = analyzer.load_data(opec_file, '欧佩克原油')
    ...
    opec = opec[opec['Date'].isin(common_dates)].reset_index(drop=True)
    ...
    print("\n步骤3: OHLC转换")
    print("-" * 60)
    opec_t = analyzer.transform(opec)
    ...
    # X = 4个因子
    X = np.hstack([opec_t, sp500_t, dollar_t, vix_t])
    Y = brent_t
    ...
    for year in years:
        ...
        try:
            ...
            results[year] = {
                'vip_opec': model.vip_scores[0],
                'vip_sp500': model.vip_scores[1],
                'vip_dollar': model.vip_scores[2],
                'vip_vix': model.vip_scores[3],
                'explained_Y': model.explained_variance['t1']['Y']
            }

            print(f"\nVIP评分:")
            print(f"  欧佩克原油: {model.vip_scores[0]:.3f} {'***' if model.vip_scores[0] > 1 else ''}")
            print(f"  标普500: {model.vip_scores[1]:.3f} {'***' if model.vip_scores[1] > 1 else ''}")
            print(f"  美元指数: {model.vip_scores[2]:.3f} {'***' if model.vip_scores[2] > 1 else ''}")
            print(f"  恐怖指数: {model.vip_scores[3]:.3f} {'***' if model.vip_scores[3] > 1 else ''}")
...
def plot_results(results, years):
    ...
    vip_opec = [results[y]['vip_opec'] for y in years]
    ...
    # 图1: 4条线
    axes[0, 0].plot(years, vip_opec, 'o-', label='欧佩克原油', lw=2, ms=6, c='#E74C3C')
    ...
    w = 0.2
    axes[0, 1].bar(x_pos - 1.5*w, vip_opec, w, label='欧佩克原油', color='#E74C3C', alpha=0.8)
    axes[0, 1].bar(x_pos - 0.5*w, vip_sp500, w, label='标普500', color='#3498DB', alpha=0.8)
    axes[0, 1].bar(x_pos + 0.5*w, vip_dollar, w, label='美元指数', color='#F39C12', alpha=0.8)
    axes[0, 1].bar(x_pos + 1.5*w, vip_vix, w, label='恐怖指数', color='#9B59B6', alpha=0.8)
    axes[0, 1].axhline(1, c='gray', ls='--')
    axes[0, 1].set_title('4因子VIP对比', fontweight='bold')
    ...
    # 图4: 热图
    mat = np.array([vip_opec, vip_sp500, vip_dollar, vip_vix]).T
    im = axes[1,1].imshow(mat, cmap='RdYlGn', aspect='auto', vmin=0, vmax=2)
    axes[1,1].set_xticks([0,1,2,3])
    axes[1,1].set_xticklabels(['欧佩克','标普500','美元指数','恐怖指数'])
    ...
    plt.tight_layout()
    plt.savefig('布伦特原油_4因子分析.png', dpi=300, bbox_inches='tight')
    print("\n✓ 图表已保存")
    plt.show()
...
def print_conclusion(results, years):
    ...
    print("\n\n" + "=" * 60)
    print("伦敦布伦特原油 4 大影响因子 —— 最终结论")
    print("=" * 60)

    factors = {
        "欧佩克原油": np.mean(vip_opec),
        "标普500": np.mean(vip_sp500),
        "美元指数": np.mean(vip_dollar),
        "恐怖指数": np.mean(vip_vix)
    }

    vip_map = {"欧佩克原油": vip_opec, "标普500": vip_sp500, "美元指数": vip_dollar, "恐怖指数": vip_vix}
    for name, score in factors.items():
        print(f"\n{name}:")
        print(f"  平均VIP: {score:.3f}")
        sig = [y for y, v in zip(years, vip_map[name]) if v > 1]
        if sig:
            print(f"  显著年份: {sig}")
...
if __name__ == "__main__":
    results = analyze(
        opec_file='../数据/欧佩克油.xlsx',          # 1
        sp500_file='../数据/美国标准普尔500指数.xlsx', # 2
        dollar_file='../数据/美元指数.xlsx',           # 3
        vix_file='../数据/恐慌指数.xlsx',              # 4
        brent_file='../数据/伦敦布伦特原油期货价格.xlsx'        # 伦敦布伦特原油
    )